  ...
  //   }
    ...
    if (that.data.btnkg == 1) {
      ...
      that.setData({
		...
		wx.request({
      url: app.IPurl +'/api/hot_subject/praise',
			data:{
    ...
    // this.setData({ show: true });
		wx.navigateTo({
			url:'/pages/shequpl_hot/shequpl?sqid='+this.data.sqid
		})
  },
	bint(e){
	...
	getdetails(id){
		...
		wx.request({
      url: app.IPurl +'/api/hot_subject/show',
			data:{
				"authorization":wx.getStorageSync('usermsg').user_token,
				"community_id":id
			},
	...
	getpl(type){
		...
		wx.request({
      url: app.IPurl +'/api/hot_comment/index',
			data:{
				"community_id":that.data.sqid,
				'page':that.data.page
			},
